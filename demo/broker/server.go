...
func (s *brokerService) Process(context context.Context, msg *proto.Msg) (res *proto.Response, err error) {
	...
	case 1:
		...
		fmt.Println(msg)
		res.Ok = true
		res.O = &proto.Response_Data{Data: []byte("消息成功放入消息队列")}
	...
	case 2:
		...
		res.O = &proto.Response_Data{Data: bytes}
		return
	...
	case 3:
		...
		for e := s.msgList.Front(); e != nil; e = e.Next() {
			if e.Value.(*proto.Msg).Id == msg.Id {
				s.msgList.Remove(e)
				s.len = s.msgList.Len()
				break
			}
		...
		res.Ok = true
		res.O = &proto.Response_Data{Data: []byte("ack删除成功")}
		s.brokerLock.Unlock()
	default:
...
func main() {
	...
	proto.RegisterBrokerServer(s, &brokerService{msgList: list.New()})
